package com.adriankwiatkowski.logcolor.colorbuilder.converters;
...
public class AnsiTextConverter extends TextConverter {
    ...
    private static final List<Pair<Integer, String>> COLOR_TABLE =
            Collections.unmodifiableList(new ArrayList<Pair<Integer, String>>() {{
    ...
    private String getTextAligned(Text text) {
        ...
        if (textAttribute != null && textAttribute.getTextAlignment() != null) {
            ...
            if (!s.equals("\n")) {
                if (s.endsWith("\n")) {
                    s = textAttribute.getTextAlignment().writeAligned(s.substring(0,
                                                                                  s.length() -
                                                                                  "\n".length()),
                                                                      textAttribute.getExtraSpace()) +
                        "\n";
                } else {
                    s = textAttribute.getTextAlignment().writeAligned(s,
                                                                      textAttribute.getExtraSpace());
                }
            }
        }
